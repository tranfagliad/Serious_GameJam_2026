...
function DrawPlayerHealth (_current_hp, _max_hp) {
	...
	draw_sprite_ext(sprHealthBadge, 0, VIEWPORT_WIDTH-20, 20, 2, 2, 0, c_white, 1);
	
	for (var _i = 0; _i < _max_hp; _i++) {
		var _missing_hp = _max_hp - _current_hp;
		var _frame = (_i >= _missing_hp) ? 0 : 1;
		
		var _draw_x = (VIEWPORT_WIDTH - 355) + (_i * 45);
		
		draw_sprite_ext(sprPlayerHealth, _frame, _draw_x, 62, 2, 2, 0, c_white, 1);
	}
}
